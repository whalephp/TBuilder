...
trait ViewSetting 
{
	...
	public function fetch($template = '', $vars = [], $replace = [], $config = [])
	{
		...
		foreach ($view_replace_str as $key=>$val){
			$this->_view_vars['extend']['extend_css'] = str_replace($key, config('view_replace_str.'.$key), $this->_view_vars['extend']['extend_css']);
			$this->_view_vars['extend']['extend_js']  = str_replace($key, config('view_replace_str.'.$key), $this->_view_vars['extend']['extend_js']);
		}
	...
	protected function assignExtendFile(){
		...
		if( $this->_view_vars['widgets_in_use'] ){
			...
			//加载组件配置文件
			$widgets_config = require_once WHALE_BUILDER_PATH . 'widgets_config.php';
			//所有启用的组件
